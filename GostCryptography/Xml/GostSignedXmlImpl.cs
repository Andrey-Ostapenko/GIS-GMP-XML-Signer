...
namespace GostCryptography.Xml
{
	sealed class GostSignedXmlImpl : SignedXml
	{
        ...
        [SecuritySafeCritical]
        public void ComputeSignatureGost(string prefix)
        {
            var signingKey = SigningKey;

            if (signingKey == null)
            {
                ComputeSignatureBase(prefix);
            }
            else
            {
                if ((SignedInfo.SignatureMethod == null) && (signingKey is Gost3410AsymmetricAlgorithmBase))
        ...
        [SecurityCritical]
        private void ComputeSignatureBase(string prefix)
        {
            // ComputeSignature();
        ...
        private void BuildDigestedReferences()
        {
            ...
            typeof(SignedXml).GetMethod("BuildDigestedReferences", BindingFlags.NonPublic | BindingFlags.Instance).Invoke(this, new object[] { });
        }
